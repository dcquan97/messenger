    ...
    socket.on("chat-attachment", (data) =>{
      if (data.groupId) {
        ...
        if (clients[data.groupId]) {
          emitNofifyToArray(clients, data.groupId, io, "response-chat-attachment", response);
        };
      }
      if (data.contactId) {
        let response = {
        ...
        if (clients[data.contactId]) {
          emitNofifyToArray(clients, data.contactId, io, "response-chat-attachment", response);
        };
      }
    });
