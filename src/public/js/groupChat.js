...
function callSearchFriends(element) {
  ...
  if (!regexKeyword.test(keyword)) {
    alertify.notify("Lỗi từ khoá tìm kiếm, chỉ cho phép chữ cái, chữ số và khoảng trống.", "error", 7);
    return false;
  }
...
function callCreateGroupChat() {
  $("#btn-create-group-chat").unbind("click").on("click", function() {
    ...
    if (countUsers.length < 2) {
      alertify.notify("Vui lòng chọn bạn bè để thêm vào nhóm, tối thiểu 2 bạn bè.", "error", 7);
      return false;
    }
    ...
    if (!regexGroupChatName.test(groupChatName) || groupChatName.length < 5 || groupChatName.length > 30) {
      alertify.notify("Vui lòng đặt tên cho cuộc trò chuyện, giới hạn trong khoản 5-30 ký tự và không chứa ký tự đặc biệt.", "error", 7);
      return false;
    };
          ...
          <div class="top">
            ...
            <span class="chat-menu-right">
                <a href="javascript:void(0)" class="number-member" data-toggle="modal">
                    <span class="show-number-member">${data.groupChat.userAmount}</span>
  ...
  socket.on("response-new-group-created", function(response) {
    ...
    let rightSideData = `
      ...
      <div class="top">
        ...
        <span class="chat-menu-right">
            <a href="javascript:void(0)" class="number-member" data-toggle="modal">
                <span class="show-number-member">${response.groupChat.userAmount}</span>
                <i class="fa fa-users"></i>
            </a>
        </span>
        <span class="chat-menu-right">
        ...
        <span class="chat-menu-right">
            <a href="javascript:void(0)" class="number-message" data-toggle="modal">
                <span class="show-number-message">${response.groupChat.messageAmount}</span>
                <i class="fa fa-comment-o"></i>
            </a>
        </span>
      </div>
